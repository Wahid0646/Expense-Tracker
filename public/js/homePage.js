...
async function addExpense() {
  try {
    ...
    const res = await axios
      .post(
        "http://localhost:3500/expense/addExpense",
        {
          date: dateStr,
...
async function getAllExpenses() {
  ...
  try {
    ...
    const res = await axios.get(
      "http://localhost:3500/expense/getAllExpenses/1",
      { headers: { Authorization: token } }
    );
...
async function paginationBtn(e) {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.get(
      `http://localhost:3500/expense/getAllExpenses/${pageNo}`,
      { headers: { Authorization: token } }
    );
...
async function deleteExpense(e) {
  try {
    ...
    if (e.target.classList.contains("delete")) {
      ...
      const res = await axios.get(
        `http://localhost:3500/expense/deleteExpense/${id}`,
        { headers: { Authorization: token } }
      );
      window.location.reload();
    }
...
async function editExpense(e) {
  try {
    ...
    if (e.target.classList.contains("edit")) {
      ...
      const res = await axios.get(
        "http://localhost:3500/expense/getAllExpenses",
        { headers: { Authorization: token } }
      );
        ...
        if (expense.id == id) {
          ...
          addExpenseBtn.addEventListener("click", async function update(e) {
            ...
            const res = await axios.post(
              `http://localhost:3500/expense/editExpense/${id}`,
              {
...
async function isPremiumUser() {
  const token = localStorage.getItem("token");
  const res = await axios.get("http://localhost:3500/user/isPremiumUser", {
    headers: { Authorization: token },
  });
